...
let pollWeb3 = function (state) {
  ...
  setInterval(() => {
    if (web3 && store.state.metamask.web3.web3Instance) {
      console.log('check web3 eth coinbase', web3.eth.coinbase)
      if (web3.eth.coinbase !== store.state.metamask.web3.coinbase && web3.eth.coinbase !== null) {
        let newCoinbase = web3.eth.coinbase
        web3.eth.getBalance(web3.eth.coinbase, function (err, newBalance) {
          if (err) {
            alert('somewhere here')
            console.log(err)
          } else {
